...
面向日常 Cinema 4D 工作流的个人 Python 脚本集。当前版本纳入 13 个已有工具。长期目标是围绕实际制作需求逐步完善到 100 个。

正式脚本统一使用 `HG_` 前缀，例如 `HG_FigureScale.py`。在 C4D 命令搜索中输入 `HG_` 可便于筛选；若仍显示旧名称，先保存工作再按需重启 C4D。原工具栏或快捷键可能需要重新关联。见 [改名映射](docs/hg-prefix-migration.md)。
