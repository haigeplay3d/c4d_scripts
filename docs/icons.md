...
- `HG_presetsbyhaigec4d.py` 使用小写同名 `HG_presetsbyhaigec4d.tif`。
...
旧版已跟踪图标可从前一个提交提取；本机另有修改前备份，未删除历史记录。此次图标替换是用户明确要求的全套重做；既有 pngforgithub 图片删除、Learn 文件删除及 HG_TagsSameColor.py 修改不混入图标提交。
